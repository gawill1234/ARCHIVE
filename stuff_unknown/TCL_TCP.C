...
int tcpRcv(ClientData clientData, Tcl_Interp *interp,
                int argc, char *argv[])
{
...
extern void server_loop();
   ...
   switch (argc) {
      ...
      default:
               Tcl_SetResult(interp,
                   "Usage: tcp_rcv [host] [port]", TCL_STATIC);
               return TCL_ERROR;
               break;
   }
   ...
   open_tcp_srv_noncon(tfd, server_loop);
   terminate_connection(tfd);

   return TCL_OK;
}

int tcpSnd(ClientData clientData, Tcl_Interp *interp,
                int argc, char *argv[])
{
char *dev_interface, *host;
...
extern int TCL_vrbs, doit(), Gsend_bytes;
   ...
   switch (argc) {
      ...
      default:
               Tcl_SetResult(interp,
                   "Usage: tcp_snd host [port]", TCL_STATIC);
               return TCL_ERROR;
               break;
   }
