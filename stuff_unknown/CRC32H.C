   ...
   for ( i = 0;  i < 256;  i++ ) {
      ...
      for ( j = 0;  j < 8;  j++ ) {
         if ( crc_accum & 0x80000000L )
            crc_accum = ( crc_accum << 1 ) ^ POLYNOMIAL_32;
         else
...
unsigned long gen_crc32(unsigned long crc_accum,
                        char *data_blk_ptr,
                        int data_blk_size)
{
   ...
   return(update_crc(crc_accum, data_blk_ptr, data_blk_size));
}
...
int i;
   ...
   for (i = 1; i < argc; i++)
      fprintf(stderr, "%u\n", gen_crc32(0, argv[i], strlen(argv[i])));
}
*/
