...
public class vServices
{
    ...
    private Integer ssPort = 7205;

    private Integer cbMaxCol = -1;
    private String cbDefName = "global";
    ...
    public vServices() throws java.lang.Exception
    {

       envInit();
       commonInit(this.srvEnv.getVelocityUrl(), null);
    ...
    private void envInit() {
        ...
        this.ssPort = Integer.valueOf(this.srvEnv.getVivPort());
    ...
    {
        //log.info("Stopping the collection broker");

        CollectionBrokerStop flushit = new CollectionBrokerStop();

        flushit.setAuthentication(this.auth);

        this.p.collectionBrokerStop(flushit);
    }

    public void collectionBrokerStop(boolean waitforit)
    {

        Integer i = 1;
        //log.info("Stopping the collection broker");
        this.collectionBrokerStop();

        try {
           while (this.collectionBrokerStatus().equals("running")) {
              Thread.sleep(3*1000);
              i += 1;
              if (i > 40) {
                 System.out.println("WARNING:");
                 System.out.println("   Waited 2 minutes for collection broker");
                 System.out.println("   to stop.  It may still be running.");
                 System.out.println("   Done waiting.");
                 return;
              }
           }
        } catch (java.lang.Exception jle) {
           System.out.println("collectionBrokerStatus() call failed from collectionBrokerStop()");
        }
    ...
    {
        //log.info("Starting the collection broker");

        CollectionBrokerStart flushit = new CollectionBrokerStart();

        flushit.setAuthentication(this.auth);

        this.p.collectionBrokerStart(flushit);
    }

    public void setCollectionBrokerMaxCollections(Integer cCnt)
    {
       this.cbMaxCol = cCnt;
    }

    public void setCollectionBrokerScopeName(String sName)
    {
       this.cbDefName = sName;
    }

    public void collectionBrokerIgnore(String ignoreCollection)
    {

        velocity.types.CollectionBrokerIgnoreCollection zzz = new velocity.types.CollectionBrokerIgnoreCollection();

        zzz.setCollection(ignoreCollection);
        zzz.setAuthentication(this.auth);

        this.p.collectionBrokerIgnoreCollection(zzz);
    ...
    public void collectionBrokerSet()
    {
        ...
        CollectionBrokerConfiguration zzz = new CollectionBrokerConfiguration();

        zzz.setMaximumCollections(this.cbMaxCol);
        zzz.setName(this.cbDefName);

        CollectionBrokerSet.Configuration sssC =
                         new CollectionBrokerSet.Configuration();

        sssC.setCollectionBrokerConfiguration(zzz);

        CollectionBrokerSet sss = new CollectionBrokerSet();
        sss.setAuthentication(this.auth);
        sss.setConfiguration(sssC);

        this.p.collectionBrokerSet(sss);

    }

    private velocity.objects.CollectionBrokerStatusResponse getCBSResp()
    {
        velocity.objects.CollectionBrokerStatusResponse zinger = null;
        String statusString = "stopped";

        //log.info("Getting search service data");
        CollectionBrokerStatus foo = new CollectionBrokerStatus();
        foo.setAuthentication(this.auth);

        try {
           velocity.types.CollectionBrokerStatusResponse resp = this.p.collectionBrokerStatus(foo);
           if (resp == null) {
              return null;
           }

           zinger = resp.getCollectionBrokerStatusResponse();
    ...
    //
    //   Valid operations on the CollectionBrokerConfiguration node:
    //      getMaximumCollections()           Integer
    //      getPreferRequests()               String
    //      getMinimumFreeMemory()            BigInteger
    //      getOvercommitFactor()             Double
    //      getIndexerOverhead()              BigInteger
    //      getCrawlerOverhead()              BigInteger
    //      getIndexerMinimum()               BigInteger
    //      getCrawlerMinimum()               BigInteger
    //      getMemoryGranularity()            BigInteger
    //      getTimeGranularity()              Long
    //      getCheckOnlineTime()              Double
    //      getCurrentStatusTime()            Double
    //      getStartOfflineTime()             Double
    //      getCheckMemoryUsageTime()         Double
    //      getPersistentSaveTime()           Double
    //      getLivePingProbability()          Double
    //      getCheckOnlineAreResponsiveTime() Double
    //      getName()                         String
    public CollectionBrokerConfiguration collectionBrokerConfiguration()
    {
        velocity.objects.CollectionBrokerStatusResponse zinger = null;

        zinger = this.getCBSResp();
        if (zinger != null) {
           return(zinger.getCollectionBrokerConfiguration());
        }

        return(null);
    }

    public String collectionBrokerStatus()
    {
        velocity.objects.CollectionBrokerStatusResponse zinger = null;

        zinger = this.getCBSResp();
        if (zinger != null) {
           return(zinger.getStatus());
        }
    ...
    public void setSearchServicePort(Integer newPort) {
       this.ssPort = newPort;
    }

    public Integer showSearchServicePort(Integer newPort) {
       return(this.ssPort);
    }

    public void setSearchServiceIP(String newIP) {
       this.IPListString = newIP;
    }

    public void resetDefaultSearchServiceIP(String newIP) {
       this.IPListString = "*";
    }

    public void addSearchServiceIP(String newIP) {
       this.IPListString = this.IPListString + " " + newIP;
    }

    public String showSearchServiceIP(String newIP) {
    ...
    public void searchServiceSet()
    {
        ...
        vsOptNode.setPort(this.ssPort);
        System.out.println("IP List:  " + this.IPListString);
        vsOptNode.setAllowIps(this.IPListString);
