...
public class myBulkLoader {
   ...
   public static void uploadFromDir(String dirName, myConnection funcJunk) {
      ...
      try {
         ...
         dcount = myDirs.length;
         for (di = 0; di < dcount; di++) {
            uploadFromDir(myDirs[di].getAbsolutePath(), funcJunk);
         }
      } catch (Exception e) {
   ...
   public static void deleteAllFolders(myConnection funcJunk,
      ...
      for (i = 0; i < listlen; i++ ) {
         ...
         deleteAllFolders(funcJunk, NewName);
      }
   ...
   public static void doTheDelete(myConnection myjunk,
      ...
      if (dowhat.equals("tree")) {
         deleteAllFolders(myjunk, mountpoint);
   ...
   public static void doTheCreate(myConnection myjunk,
      ...
      if (dowhat.equals("list")) {
         ...
         uploadFromDir(mountpoint, myjunk);
      } else {
         System.out.println("WTF is that?  " + dowhat);
      }
   ...
   public static void main(String[] args) {
      ...
      //  Operation variables
      Boolean createFlag, deleteFlag, updateFlag, dumpMime, fullDump, qc;
      ...
      qc = createFlag = deleteFlag = updateFlag = dumpMime = fullDump = false;
      qc = true;
      quantity = 0;
      ...
      if (createFlag) {
         doTheCreate(myjunk, dowhat, itemname, mountpoint, quantity);
      } else if (deleteFlag) {
         doTheDelete(myjunk, dowhat, itemname, mountpoint, quantity);
      } else if (updateFlag) {
         ...
         if ( dumpMime ) {
            ...
            System.out.println(root.getId().toString());
            try {
