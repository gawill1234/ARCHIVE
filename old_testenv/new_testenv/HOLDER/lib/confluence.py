...
class CONFLUENCE:
    ...
    def addAnonymousSpacePermission(self, spaceKey=None, perm=None):
    ...
    def getAvailablePermissions(self):
        ...
        try:
            ...
            if self.__verbose:
                print 'Error function:  getAvailablePermissions'
                print 'Error Confluence function:  confluence2.getPermissions()'
                print 'Confluence error: ',
                print sys.exc_info()
            permList = []
    ...
    #
    #   Vector example of test_data locked by admin to be viewable only by admin
    #   PAGE [{'lockedBy': 'admin', 'lockType': 'View'}]
    #   CONTENT [{'contentPermissions': [{'userName': 'admin', 'type': 'View'}], 'type': 'View'}]
    #   PAGE [{'lockedBy': 'admin', 'lockType': 'View'}, {'lockedBy': 'bubbles', 'lockType': 'View'}, {'lockedBy': 'admin', 'lockType': 'Edit'}, {'lockedBy': 'bubbles', 'lockType': 'Edit'}]
    #
    def getPagePermissions(self, spaceKey=None, pageTitle=None, pageId=None):

        if ( pageId is None ):
            if ( spaceKey is not None and pageTitle is not None ):
                myPage = self.getPage(spaceKey, pageTitle)
                if ( myPage is not None and myPage != '' and myPage != {} ):
                   pageId = myPage['id']
                else:
                   return []
    ...
    #   CONTENT [{'contentPermissions': [{'userName': 'admin', 'type': 'View'}], 'type': 'View'}]
    #
    def getContentPermissions(self, spaceKey=None, pageTitle=None, pageId=None):

        if ( pageId is None ):
            if ( spaceKey is not None and pageTitle is not None ):
                myPage = self.getPage(spaceKey, pageTitle)
                if ( myPage is not None and myPage != {} ):
                   pageId = myPage['id']
                else:
                   pageId = None
    ...
    #   CONTENT [{'contentPermissions': [{'userName': 'admin', 'type': 'View'}], 'type': 'View'}]
    #
    def buildContentPermissions(self, inUser=None, permType='View'):

       userPerm = {}
       userPerm['userName'] = inUser
       if ( permType is None ):
          userPerm['type'] = 'View'
       else:
          userPerm['type'] = permType

       cpList = [userPerm]

       contentPerm = {}
       contentPerm['contentPermissions'] = cpList
       contentPerm['type'] = permType

       permList = [contentPerm]
    ...
    def setContentPermissions(self, spaceKey=None, pageTitle=None,
                                    user=None, permType=None, pageId=None):
        ...
        allPerms = self.buildContentPermissions(user, permType)
        permType = 'Edit'

        if ( pageId is not None and permType is not None ):
            try:
    ...
    #   nPage would probably have come out of initializePage()
    #
    def updatePage(self, nPage = {}, updateOptions = {}):

        try:
            result = self.__server.confluence2.updatePage(self.__token,
                                   nPage, updateOptions)
        except:
            if self.__verbose:
                print 'Error function:  updatePage'
                print 'Error Confluence function:  confluence2.updatePage()'
                print 'Confluence error: ',
                print sys.exc_info()
            result = {}
    ...
    def commentExistingPage(self, spaceKey, pageTitle, comment):

        try:
            myPage = self.getPage(spaceKey, pageTitle)
            myPage['content'] = comment
            updateOptions = {}
            updateOptions['versionComment'] = comment
            updateOptions['minorEdit'] = False
        except:
            print 'Error function:  renamePage'
            print '    Getting page data failed.'
            print '    ', spaceKey, pageTitle, newTitle
            return {}

        pageData = self.updatePage(myPage, updateOptions)
        return pageData


    def renamePage(self, spaceKey, pageTitle, newTitle):

        try:
            myPage = self.getPage(spaceKey, pageTitle)
            myPage['title'] = newTitle
        except:
            print 'Error function:  renamePage'
            print '    Getting page data failed.'
            print '    ', spaceKey, pageTitle, newTitle
        ...
        pageData = self.updatePage(myPage)
        return pageData
    #
    ...
    def watchASpace(self, SpaceKey=None):

        if ( SpaceKey is None ):
            return False

        try:
            MyRes = self.__server.confluence2.watchSpace(self.__token, SpaceKey)
        except:
            print 'Error function:  watchASpace()'
            print 'Error Confluence function:  confluence2.watchSpace()'
            print 'Confluence error: ', sys.exc_info()
            MyRes = False
...
if (__name__ == '__main__'):
    #def setContentPermissions(self, spaceKey=None, pageTitle=None,
    #                                user=None, permType='View', pageId=None):
    server_url = 'http://172.16.10.81:8090/rpc/xmlrpc'
    #cnfl = CONFLUENCE(server_url, 'bubbles', 'squirrel')
    cnfl = CONFLUENCE(server_url)
    cnfl.setContentPermissions('testenv', 'PPT', 'princess_morbucks', 'View', None)
    himcnfl = CONFLUENCE(server_url, 'princess_morbucks', 'squirrel')
    print himcnfl.getContentPermissions('testenv', 'PPT')
    print himcnfl.getPagePermissions('testenv', 'PPT')
    print cnfl.getContentPermissions('testenv', 'PPT')
    print cnfl.getPagePermissions('testenv', 'PPT')
    print cnfl.getAvailablePermissions()
    print himcnfl.getAvailablePermissions()
    #himcnfl.watchASpace('testenv')
    #cnfl.deleteAttachment('testenv', 'PPT', 'boeing.ppt')
    #def removeAnonymousSpacePermission(self, spaceKey=None, perm=None):
    #print cnfl.newUser(userName='mojojojo', pw='itisi-mojojojo')
    #cnfl.renamePage('testenv', 'PPT_upd','PPT')
    #cnfl.commentExistingPage('testenv', 'test_data','SMURF VOMIT')
    #cnfl.dumpIt()
