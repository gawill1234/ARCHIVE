...
class SimpleIndexerKill ( Thread ):
   ...
   def run(self):
      ...
      tname = 'api-sc-index-atomic-12.kill_thread'
      ...
      while ( gogo ):
         ...
         for item in query_list:
            ...
            #
            rndtime = g.randint(0, 15)

            print tname, ":  Indexer to die in", rndtime, "seconds"

            time.sleep(rndtime)

            print tname, ":  Killing the indexer from kill thread"
            self.yy.api_sc_indexer_stop(collection=self.collection,
                                        killit='true')
            if ( item is None ):
               myq = ''
            else:
            ...
            print tname, ":  Run query.  Will restart indexer if not running"
            beginning = time.time()
...
if __name__ == "__main__":
   ...
   collection_name2 = "scia-12"
   cfile = collection_name2 + '.xml'
   tname = "api-sc-index-atomic-12"

   ##############################################################
   print tname, ":  ##################"
   print tname, ":  INITIALIZE"
   print tname, ":  index-atomic test 12"
   print tname, ":     index atomic with partial option and"
   print tname, ":     the indexer being killed periodically."
   ...
   gogo = False
   doit = SimpleIndexerKill(collection=collection_name2, i=1)
   doit.start()
   gogo = True
