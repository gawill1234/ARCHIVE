...
def dofinalauditlog(collection=None, tname=None, filecount=0,
   ...
   if ( totalcount < tcnt2 ):
      fail5 = 1
      print tname, ":  total count and entrycount do not match"
      print tname, ":  ", totalcount, tcnt2
   if ( transmatch < tcnt2 ):
      fail5 = 1
      print tname, ":  transaction count and successcount do not match"
      print tname, ":  ", transmatch, tcnt2
...
def do_crawl_delete(crawl_urls=None, filename=None,
   ...
   if ( use_origin ):
      originator = "api-sc-index-atomic-11"
   else:
      originator = None
...
def do_crawl_urls(crawl_urls=None, idx_atomic=None, 
   ...
   if ( get_max(100) > 75 or
      ...
      if ( idx_atomic is not None ):
         enqueue_it(crawl_urls, "do_crawl_urls-2")
         idx_atomic = None
         crawl_urls = build_schema_node.create_crawl_urls(
                      synchronization='enqueued')
         did_enqueue = 0
...
def do_filelist(crawl_urls, idx_atomic, filelist=None,
   ...
   if ( did_enqueue == 0 ):
      ...
      crawl_urls = None
      idx_atomic = None
   crawl_urls = flush_partial(use_origin)
   enqueue_it(crawl_urls, "do_filelist-2")
   #
   #   this routine puts in a duplicate partial termination or an
   #   empty crawl-urls node, depending on the value of crawl-urls.
   #
   do_double_take(crawl_urls)
   crawl_urls = None
   idx_atomic = None
...
if __name__ == "__main__":
   ...
   collection_name2 = "scia-11"
   cfile = collection_name2 + '.xml'
   tname = "api-sc-index-atomic-11"

   ##############################################################
   print tname, ":  ##################"
   print tname, ":  INITIALIZE"
   print tname, ":  index-atomic test 11"
