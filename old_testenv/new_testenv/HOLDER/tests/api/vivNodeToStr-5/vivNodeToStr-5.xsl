...
<vce version="4.0">
<application name="vivNodeToStr-5" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  vivNodeToStr-5 -- two level path<br/><br/>
      <b>Function:</b>  viv:node-to-str($testNode,true())<br/><br/>
      <b>Variable Values:</b>
      <ul>
      <li>
      <b>testNode:</b>   testnode2/name
      </li>
      </ul>
      <b>Expected Result:</b>  &lt;name&gt; dummy &lt;/name&gt;<br/><br/>

      <xsl:variable name="junk" select="//testset/test" as="element()+"/>

      <xsl:for-each select="$junk">

         <xsl:variable name="testNode" select="testnode2/name"/>
         <xsl:variable name="expected" select="string('&lt;name&gt; dummy &lt;/name&gt;')" as="xs:string"/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="contains(normalize-space($result), normalize-space($expect))">
            <br/>
            <font color="green">
            <b>vivNodeToStr-5:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>vivNodeToStr-5:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
