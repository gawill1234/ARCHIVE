...
<vce version="4.0">
<application name="vivMatch-8" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  match of odd character strings, ignore case<br/><br/>
      ...
      <xsl:variable name="matchString" select="string('!@#$ %^* ()-=+ |?;:.,')"/>
      <xsl:variable name="regexStr" select="string('!@#$.*')"/>
      <xsl:variable name="expected" select="string('!@#$ %^* ()-=+ |?;:.,')"/>
