...
<vce version="4.0">
<application name="fileCreateDir-3" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  Create a directory that already exists<br/><br/>
      ...
      <xsl:variable name="dir1" select="string('tubersnzots')"/>
      <xsl:variable name="expected" select="true()"/>
      <xsl:variable name="resvaltmp" select="file:create-directory($dir1)"/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="$result = $expect">
            <br/>
            <font color="green">
            <b>fileCreateDir-3:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>fileCreateDir-3:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
