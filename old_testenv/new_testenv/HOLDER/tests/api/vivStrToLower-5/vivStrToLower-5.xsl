...
<vce version="4.0">
<application name="vivStrToLower-5" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  vivStrToLower-5 -- alphanum string, uppercase alpha to lower case<br/><br/>
      <b>Function:</b>  viv:str-to-lower($string1)<br/><br/>
      <b>Variable Values:</b>
      <ul>
      <li>
      <b>string1:</b>   ABCD1234
      </li>
      </ul>
      <b>Expected Result:</b>  abcd1234<br/><br/>
      <xsl:variable name="string1" select="string('ABCD1234')"/>
      <xsl:variable name="expected" select="string('abcd1234')"/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="$result = $expect">
            <br/>
            <font color="green">
            <b>vivStrToLower-5:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>vivStrToLower-5:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
