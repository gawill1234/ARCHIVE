...
<vce version="4.0">
<application name="vivNodeToStr-6" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  vivNodeToStr-6 -- three level path with same path name at two levels<br/><br/>
      <b>Function:</b>  viv:node-to-str($testNode,true())<br/><br/>
      <b>Variable Values:</b>
      <ul>
      <li>
      <b>testNode:</b>   testnode4/name/whatrot
      </li>
      </ul>
      <b>Expected Result:</b>  &lt;whatrot&gt; innerrot &lt;/whatrot&gt;<br/><br/>

      <xsl:variable name="junk" select="//testset/test" as="element()+"/>

      <xsl:for-each select="$junk">

         <xsl:variable name="testNode" select="testnode4/name/whatrot"/>
         <xsl:variable name="expected" select="string('&lt;whatrot&gt; innerrot &lt;/whatrot&gt;')" as="xs:string"/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="contains(normalize-space($result), normalize-space($expect))">
            <br/>
            <font color="green">
            <b>vivNodeToStr-6:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>vivNodeToStr-6:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
