...
class SimpleCrawlerKill ( Thread ):
   ...
   def __init__(self, collection=None, i=0):
      ...
      self.g = random.Random(time.time())
   ...
   def run(self):
      ...
      tname = 'api-sc-index-atomic-13.kill_thread'
      ...
      while ( gogo ):
         ...
         for item in query_list:
            ...
            #
            #   Random time between kills of the indexer
            #   Random value of from 0 to 90 seconds.
            #
            rndtime = g.randint(0, 50)

            print tname, ":  Crawler to die in", rndtime, "seconds"

            time.sleep(rndtime)

            print tname, ":  Killing the crawler from kill thread"
            self.yy.api_sc_crawler_stop(collection=self.collection,
                                        killit='true')
            if ( item is None ):
               myq = ''
            else:
            ...
            #
            #   Sleep a bit.  Make it a toss up whether it is the query or
            #   the cralwer which restarts the indexer.  From above,
            #   coube be 0, 1 or 2 seconds.
            #
            time.sleep(waittime)

            print tname, ":  Restart crawler"
            self.yy.api_sc_crawler_start(collection=self.collection)
...
def wait_and_query(xx, collection_name2, num):

   xx.wait_for_idle(collection=collection_name2)

   resp = yy.api_qsearch(xx=xx, source=collection_name2, query='', num=num)
...
def current_number(dir):
   ...
   global fileadds, filedels, currentadds, currentdels
   global tname, currlist, fulllist
   ...
   print tname, ":        FILES IN LIST   :", fulllist
   print ""
   ...
   print tname, ":        FILES IN LIST   :", currlist
   print ""
...
def do_double_take(crawl_urls):
   ...
   enqueue_it(crawl_urls, "do_double_take")

   return

def enqueue_it(crawl_urls=None, idx_atomic=None, 
               kludge=None, use_id=None):

   global dumpofd, dumpresp, yy, collection_name2, did_enqueue, dopartial

   if ( dopartial > 0 ):
      idx_atomic = build_schema_node.modify_index_atomic(
                                     partial="partial",
                                     index_atomic=idx_atomic)
   else:
   ...
   dumpofd.write( "<NEXT_NODE_FROM>")
   dumpofd.write( kludge )
   dumpofd.write( ":::" )
   dumpofd.write( "%s" % dopartial )
   dumpofd.write( "</NEXT_NODE_FROM>")
   ...
   try:
      ...
      print "FIRST ENQUEUE ATTEMPT FAILED, RETRYING ..."
      try:
         time.sleep(2)
         resp = yy.api_sc_enqueue_xml(
                          collection=collection_name2,
         ...
         print "SECOND ENQUEUE ATTEMPT FAILED, CONTINUING ..."
   ...
   if ( resp is not None ):
      ...
      dumpresp.write( "RESPONSE(resp) was NONE(i.e., empty)" )
...
def do_crawl_urls(crawl_urls=None, idx_atomic=None, 
   ...
   global enqueueid, dopartial, did_enqueue
   ...
   if ( crawl_urls is None ):
      ...
      if ( idx_atomic is None ):
         ...
         if ( get_max(100) > 75 ):
            dopartial -= 1
            enqueue_it(crawl_urls, idx_atomic, "do_crawl_urls-2", use_id)
            idx_atomic = None
            crawl_urls = build_schema_node.create_crawl_urls(
                         synchronization='enqueued')
...
def do_filelist(crawl_urls, idx_atomic, filelist=None,
   ...
   dopartial = 0
   enqueue_it(crawl_urls, idx_atomic, "do_filelist-1", use_id)
   crawl_urls = None
   idx_atomic = None
   ...
   dopartial = 0
   enqueue_it(crawl_urls, idx_atomic, "do_filelist-3", use_id)
   crawl_urls = None
   idx_atomic = None
...
def do_enqueues(dir, use_origin=False, use_id=False, 
   ...
   global enqueueid, currentadds, currentdels, currlist, fulllist
...
if __name__ == "__main__":
   ...
   global enqueueid, fileadds, filedels, fail5, fulllist
   ...
   collection_name2 = "scia-13"
   cfile = collection_name2 + '.xml'
   tname = "api-sc-index-atomic-13"

   ##############################################################
   print tname, ":  ##################"
   print tname, ":  INITIALIZE"
   print tname, ":  index-atomic test 13"
   print tname, ":     index atomic with partial option and"
   print tname, ":     the crawler being killed periodically."
   ...
   gogo = False
   doit = SimpleCrawlerKill(collection=collection_name2, i=1)
   doit.start()
   gogo = True
   ...
   #   This should be valid as there should be no dups on my file list.
   #
   indexedcount = wait_and_query(xx, collection_name2, num=50)
   ...
   #   This should be valid as there should be no dups on my file list.
   #
   indexedcount = wait_and_query(xx, collection_name2, num=5000)
