...
results = TestResults.new('Enqueue data to collection broker and continuously ',
...
thread = Thread.new {
  (0...COUNT).map {|i| 
    ok = c2.broker_enqueue_xml(wrap(CRAWL_URL % 
...
thread = Thread.new {
  (0...COUNT).map {|i| 
    ok = c2.broker_enqueue_xml(wrap(CRAWL_DELETE % i))
    if not ok
      results.add(false,
