...
def test_pipeline_size
  ...
  @test_results.add(enqueue_response_matches_pipeline_size(CRAWL_DELETE, 1) == true, 
...
def test_throttle_id
  ...
  setup
  msg "  Enqueuing a crawl-url"
  @test_results.add(enqueue_response_has_throttle_id(CRAWL_URL_1) == false, 
  ...
  msg "  Enqueuing a crawl-url"
  @test_results.add(enqueue_response_has_throttle_id(CRAWL_URL_1.sub("dummy-a", "dummy-c")) == true, 
                    "an enqueue at the enqueue-high-watermark results in a throttle-id")

  msg "  Sleeping 20 seconds"
  sleep 20
  @test_results.add(enqueue_response_matches_throttle_id(CRAWL_URL_1.sub("dummy-a", "dummy-d"), "1") == true, 
  ...
  enqueue_response_has_throttle_id(CRAWL_URL_1.sub("dummy-a", "dummy-b")) 
  msg "  Enqueuing a crawl-url"
  @test_results.add(enqueue_response_matches_throttle_id(CRAWL_URL_1.sub("dummy-a", "dummy-c"), "2") == true, 
