...
def create_server_file(collection_list, other_server_collection, lightcrawl,
   ...
   root_dir = os.getenv('TEST_ROOT', '../dstidx-common6')
   ...
   if ( lightcrawl ):
      filename = root_dir + '/tests/distributed-search/dstidx-common6/' + filename
   else:
      filename = root_dir + '/tests/distributed-search/dstidx-common6/' + filename
...
def create_client_file(my_server, my_server2, current_client,
   ...
   if ( lightcrawl ):
      filename = root_dir + '/tests/distributed-search/dstidx-common6/purebaselgtxml'
   else:
      filename = root_dir + '/tests/distributed-search/dstidx-common6/purebasexml'
...
def check_data(srvr, server_collection, clnt, verbose, Cexpected, Sexpected):
...
if __name__ == "__main__":
   ...
   root_dir = os.getenv('TEST_ROOT', '../dstidx-common6')
   test_root = root_dir + '/tests/distributed-search/dstidx-common6/'
   ...
   #   Define options
   #
   opts, args = getopt.getopt(sys.argv[1:], "I:S:C:vkm:T:", ["server=", "client=", "verbose", "keepit", "testname=", "indexatomic="])
   #
   #
   #####################################################################
   #
   ...
   clnt_updates = test_root + 'clnt_updates'
  ...
  ##############################################################
   ...
   tfail2 = tfail = 0
   tfail = check_data(srvr, server_collection, clnt, verbose,
                      expectedcnt, expectedcnt)
   tfail2 = check_data(srvr2, server_collection2, clnt, verbose,
                       expectedcnt, expectedcnt)
   ...
   thing1 = doTheEnqueue(srvr2_updates, srvr2, server_collection2, False, doia2)

   zooboo.append(thing)
   zooboo.append(thing1)
   ...
   for item in clnt:
      client_collection = collection_list[cnum]
      thing = doTheEnqueue(clnt_updates, item, client_collection, False, doia1)
      thing.start()
      thing.join()
      cnum += 1
   ...
   print tname, ":  ##################"
   print tname, ":  DELETE 1"
   print tname, ":     Delete all of the clients and reinstall."
   print tname, ":     They should rebase against the servers."
   ...
   if ( ( tfail + tfail2 ) == 0 ):
      print tname, ":  Delete 1 case passed"
   else:
      print tname, ":  Delete 1 case failed"

   print tname, ":  ##################"
   print tname, ":  DELETE 2"
   print tname, ":     Delete one of the servers."
   print tname, ":     Since it does its own crawl, data will be"
   print tname, ":     present in the collection disabling a rebase."

   srvr2.vapi.api_sc_crawler_stop(collection=server_collection2, killit='true')
   srvr2.vapi.api_sc_indexer_stop(collection=server_collection2, killit='true')
   srvr2.vapi.api_sc_delete(collection=server_collection2)

   srvr2.vapi.api_sc_create(collection=server_collection2,
                            based_on='default-push')
   server2_file = server_collection2 + '.xml'
   srvr2.vapi.api_repository_update(xmlfile=server2_file)
   srvr2.vapi.api_sc_crawler_start(collection=server_collection2)
   ...
   if ( ( tfail + tfail2 ) == 0 ):
      print tname, ":  Delete 2 case passed"
   else:
      print tname, ":  Delete 2 case failed"

   print tname, ":  ##################"
   print tname, ":  DELETE 3"
   print tname, ":     Delete all of the clients again."
   print tname, ":     It should rebase against the good server."
   print tname, ":     The previously deleted server will be out"
   print tname, ":     of step with the client so should not be used."
   ...
   if ( ( tfail + tfail2 ) == 0 ):
      print tname, ":  Delete 3 case passed"
   else:
      print tname, ":  Delete 3 case failed"
