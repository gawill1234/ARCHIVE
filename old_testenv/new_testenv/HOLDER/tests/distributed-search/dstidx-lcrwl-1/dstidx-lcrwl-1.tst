...
#
#   This test uses the light crawler for all collections.
#
...
def usage():
   ...
   print "dstidx-lcrwl-1.tst -S <server> -C <client> -O <outer client>"
   ...
   print "   Example cmd:  dstidx-lcrwl-1.tst -S testbed14 -C testbed15"
   print "                                    -O testbed16-3"
...
if __name__ == "__main__":
   ...
   cmdstring = "cat baseclntxml | sed -e \'s;REPLACE__MIDDLE__SERVER;" + fsn + ";g\' > orclclnt.xml"
   p = subprocess.Popen(cmdstring, shell=True)
   os.waitpid(p.pid, 0)
   ...
   print tname, ":  while using the light crawler."
