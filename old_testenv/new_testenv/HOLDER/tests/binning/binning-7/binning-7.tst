...
def doquery(xx, collection_name2, myquery, mybin):

   resp = yy.api_qsearch(xx=xx, source=collection_name2,
                         query=myquery, binstate=mybin)
...
if __name__ == "__main__":
   ...
   idxcnt = doquery(xx, collection_name, "Battleship", "foo==Battleship")
   print tname, ":  CASE 1, Actual  , ", idxcnt
   print tname, ":          Expected,  62"
   if ( idxcnt != 62 ):
      fail += 1
   ...
   idxcnt = doquery(xx, collection_name, "Battleship", "foo==Battleship\r")
