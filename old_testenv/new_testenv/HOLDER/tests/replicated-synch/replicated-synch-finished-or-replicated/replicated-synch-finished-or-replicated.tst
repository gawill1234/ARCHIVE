...
def check_audit_log(c, results, status, expectedFinished, expectedReplicated)
  # Wait as long as ten minutes (should this be longer?)
  expected = expectedFinished + expectedReplicated
  ...
  finished_a = success_a.select {|e| not e.key?('replicated') }
  replicated_a = success_a.select {|e| e['replicated'] =~ /^replicated/ }
  finished_count = finished_a.length
  replicated_count = replicated_a.length

  results.add(replicated_count == expectedReplicated &&
              finished_count == expectedFinished,
              "Found #{finished_count} finished entries and #{replicated_count} replicated entries in log.",
              "Found #{finished_count} finished entries and #{replicated_count} replicated entries in log, expected #{expectedFinished} and #{expectedReplicated}.")

  if audit_token and ! audit_token.empty?
    c.audit_log_purge(audit_token)
  end

  if (replicated_count != expectedReplicated) or 
    (finished_count != expectedFinished) or PRINT_AUDIT_LOG_IN_ANY_CASE
...
msg "Check that email appears in the audit logs on all nodes."

# Server 1
msg "Checking audit log on server1 #{server1_addr}."
check_audit_log(server1_c, results, "successful", 2, 2)

# Server 2
msg "Checking audit log on server2 #{server2_addr}."
check_audit_log(server2_c, results, "successful", 2, 2)

# Client 1
msg "Checking audit log on client1 #{client1_addr}."
check_audit_log(client1_c, results, "successful", 2, 0)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "successful", 2, 0)
...
msg "Make sure that the index-atomic URLs appear only once on clients and twice on servers."

# Server 1
msg "Checking audit log on server1 #{server1_addr}."
check_audit_log(server1_c, results, "successful", 1, 1)

# Server 2
msg "Checking audit log on server2 #{server2_addr}."
check_audit_log(server2_c, results, "successful", 1, 1)

# Client 1
msg "Checking audit log on client1 #{client1_addr}."
check_audit_log(client1_c, results, "successful", 1, 0)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "successful", 1, 0)
...
results.add(! server1_c.enqueue_xml(to_enqueue),
            "Failed to enqueue unconvertable document.",
            "Enqueue succeeded, should have return an error.")

msg "Make sure a failed entry exists on all nodes."

# Server 1
msg "Checking audit log on server1 #{server1_addr}."
check_audit_log(server1_c, results, "unsuccessful", 1, 1)

# Server 2
msg "Checking audit log on server2 #{server2_addr}."
check_audit_log(server2_c, results, "unsuccessful", 1, 1)

# Client 1
msg "Checking audit log on client1 #{client1_addr}."
check_audit_log(client1_c, results, "unsuccessful", 1, 0)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "unsuccessful", 1, 0)
...
msg "Make sure a deleted entry exists on all nodes."
...
# Server 2
msg "Checking audit log on server2 #{server2_addr}."
check_audit_log(server2_c, results, "successful", 1, 2)
...
msg "Deleting document http://#{EMAIL2} from server1"
