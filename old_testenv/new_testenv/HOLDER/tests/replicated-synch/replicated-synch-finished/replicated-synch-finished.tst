...
msg "Check that email appears in the audit log on all nodes."
...
check_audit_log(client1_c, results, "successful", COUNT)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "successful", COUNT)
...
# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "successful", 1)

msg "Enqueuing a document for which conversion will fail."
...
msg "Make sure a failed entry exists on all audit logs."
...
# Client 1
msg "Checking audit log on client1 #{client1_addr}."
check_audit_log(client1_c, results, "unsuccessful", 1)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "unsuccessful", 1)
...
results.add(server2_c.enqueue_xml(to_enqueue),
            "Deleted document with key #{key(0)}.",
            "Failed to delete document with key #{key(0)}.")

msg "Make sure a deleted entry exists on all nodes:"
msg "one entry (url delete) on master (server2)"
msg "and two entries (url and vse-key) on each replica"
...
# Client 1
msg "Checking audit log on client1 #{client1_addr}."
check_audit_log(client1_c, results, "successful", 2)

# Client 2
msg "Checking audit log on client2 #{client2_addr}."
check_audit_log(client2_c, results, "successful", 2)
