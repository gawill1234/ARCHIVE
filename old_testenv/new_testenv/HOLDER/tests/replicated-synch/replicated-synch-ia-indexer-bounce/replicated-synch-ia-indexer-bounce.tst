...
class IndexAtomicUrlLocator
  def url_for_entry(entry)
    entry.child.child['url']
  end
end

url_locator = IndexAtomicUrlLocator.new

server1_hash = UrlHash.new(SERVER1_NAME, results, url_locator)
server2_hash = UrlHash.new(SERVER2_NAME, results, url_locator)
client1_hash = UrlHash.new(CLIENT1_NAME, results, url_locator)
client2_hash = UrlHash.new(CLIENT2_NAME, results, url_locator)
...
class IndexAtomicCreator
  CRAWL_URL = <<-HERE
<index-atomic synchronization="indexed-no-sync" >
...
s1_enq = IgnoreAllFailuresEnqueuer.new(server1_c2, IndexAtomicCreator.new)
s2_enq = IgnoreAllFailuresEnqueuer.new(server2_c2, IndexAtomicCreator.new)
