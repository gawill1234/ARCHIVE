...
###
   ...
   VCOLLECTION="wildcard-5"
   DESCRIPTION="basic crawl verifying that the new wildcard dictionary is right"
...
repository_delete -t application -n wildcard-5-delete-dictionary
repository_delete -t dictionary -n wildcard-5-dictionary
repository_delete -t application -n wildcard-5-axl-hack
#
#   Done with try.
#
#####################################################################
...
repository_import -F wildcard-5-delete-dictionary.xml -N
run_velocity -A wildcard-5-delete-dictionary

if [ "$majorversion" -lt "8" ]; then
   repository_import -F wildcard-5-dictionary_7.xml -N
else
   repository_import -F wildcard-5-dictionary.xml -N
fi

repository_import -F wildcard-5-axl-hack.xml -N
...
run_velocity -A wildcard-5-axl-hack
#./build_dict.py
echo "#######################"
...
vivdir=`vivisimo_dir`
if [ "$VIVTARGETOS" != "windows" ]; then
   vfullpath="$vivdir/data/dictionaries/wildcard-5-dictionary/wildcard.dict"
else
   vfullpath="$vivdir\\data\\dictionaries\\wildcard-5-dictionary\\wildcard.dict"
fi
