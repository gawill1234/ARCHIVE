...
<vce version="4.0">
<application name="vivNodeToStr-13" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  vivNodeToStr-13 -- three level path and back two levels<br/><br/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="contains(normalize-space($result), normalize-space($expect))">
            <br/>
            <font color="green">
            <b>vivNodeToStr-13:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>vivNodeToStr-13:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
