...
#   index-atomic node.  Variants to index-atomic itself are addressed
#   in another test.
#
...
def dofinalauditlog(collection=None, tname=None, filecount=0,
   ...
   print tname, ":(final)       crawl-urls,            ", cucount
...
def do_crawl_delete(crawl_urls=None, filelist=None, synchro=None,
   ...
   if ( use_origin ):
      originator = "api-sc-index-atomic-2"
   else:
      originator = None
   ...
   for item in filelist:
      ...
      crawl_delete = build_schema_node.create_crawl_delete(url=url,
                  ...
                  originator=originator, enqueueid='%s' % enqueueid,
                  vsekey=vsekey )
   ...
   for path, dirs, files in os.walk(dir):
      ...
      for name in files:
         ...
         if ( curcnt >= lcnt ):
            ...
            #
            #   If both of these are true, there should be no transaction
            #   at all.  So, no appearance in audit log.
            #
            if ( use_url and use_vsekey ):
               dtransaction_count -= 1
...
if __name__ == "__main__":
   ...
   ftransaction_count = 0
   dtransaction_count = 0
   maxcount = 10
   i = 0
   num = 500
   clustercount = 10
   ...
   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-2, origin only"
   dir = "/testenv/test_data/law/F2/190"
   filecount2 = do_enqueues(dir, use_origin=True, use_id=False)
   print tname, ":  ##################"

   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-2, enqueue-id only"
   dir = "/testenv/test_data/law/F2/191"
   filecount3 = do_enqueues(dir, use_origin=False, use_id=True, beginid=0)
   print tname, ":  ##################"

   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-2, origin and enqueue-id"
   ...
   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-4, Attempt to delete by originator"
   ...
   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-4, Attempt to delete by enqueue-id"
   print tname, ":     Every delete here should fail"
   dir = "/testenv/test_data/law/F2/191"
   ...
   print tname, ":  ##################"
   print tname, ":  CASE ia-crawlia-4, Attempt to delete by origin and id"
   print tname, ":     Every delete here should fail"
   dir = "/testenv/test_data/law/F2/192"
   ...
   print tname, ":  ##################"
   print tname, ":  CASE ia-vsedelete-3, ia-vsedelete-4"
   ...
   dir = "/testenv/test_data/law/F2/191"
   deletecount3 = do_deletes(dir, use_origin=False, use_id=True,
                            use_url=True, use_vsekey=False, beginid=0)
   ...
   if ( currentcount != indexedcount ):
      ...
      print tname, ":  total results             = ", indexedcount
      print tname, ":  current count             = ", currentcount
      print tname, ":  failed deletes(expected)  = ", deletecount1c
      print tname, ":  enqueue transactions      = ", ftransaction_count
      print tname, ":  delete transactions       = ", dtransaction_count
   else:
      print tname, ":  total results and filecount are correct at ",currentcount
      print tname, ":  total results             = ", indexedcount
      print tname, ":  current count             = ", currentcount
