...
<vce version="4.0">
<application name="vivReplace-8" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <b>Expected Result:</b>  The slow Brown fox jumps over the slow lazy dog<br/><br/>
      <xsl:variable name="string1" select="string('The Quick Brown fox jumps over the quick lazy dog')"/>
      <xsl:variable name="string2" select="string('q[uv][hi]ck')"/>
      <xsl:variable name="string3" select="string('slow')"/>
      <xsl:variable name="expected" select="string('The slow Brown fox jumps over the slow lazy dog')"/>
