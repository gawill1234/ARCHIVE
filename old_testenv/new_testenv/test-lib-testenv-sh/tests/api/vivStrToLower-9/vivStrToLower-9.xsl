...
<vce version="4.0">
<application name="vivStrToLower-9" modified-by="gary_testing" max-elt-id="1"
  modified="1209666370"
>
  ...
  <print>
   ...
   <xsl:template match="/">
      ...
      <br/><br/>
      <b>Description:</b>  vivStrToLower-9 -- mixed string, mixed case<br/><br/>
      ...
      <b>Expected Result:</b>  the colon (:).<br/><br/>
      <xsl:variable name="expected" select="string('the colon (:).')"/>
      <xsl:variable name="resval" select="viv:str-to-lower('THE Colon (:).')"/>
   ...
   <xsl:template name="PassFail">
      ...
      <xsl:choose>
         <xsl:when test="$result = $expect">
            <br/>
            <font color="green">
            <b>vivStrToLower-9:  TEST PASSED</b>
            </font>
            <br/>
         </xsl:when>
         <xsl:otherwise>
            <br/>
            <font color="red">
            <b>vivStrToLower-9:  TEST FAILED</b>
            </font>
            <br/>
         </xsl:otherwise>
      </xsl:choose>
