...
<application name="set-var-5" modified-by="williams@vivisimo" max-elt-id="1" modified="1209571541">

<declare name="myx" type="boolean"/>
<set-var name="myx"><![CDATA[true]]></set-var>
               ...
               <xsl:template match="/">
                  <xsl:choose>
                     <xsl:when test="$myx = true()">
                        set-var-5:  TEST PASSED
                     </xsl:when>
                     <xsl:otherwise>
                        myx value(UP):  <value-of-var name="myx"/><br/>
                        set-var-5:  TEST FAILED
                     </xsl:otherwise>
                  </xsl:choose>
               </xsl:template>
            ]]></parser>
