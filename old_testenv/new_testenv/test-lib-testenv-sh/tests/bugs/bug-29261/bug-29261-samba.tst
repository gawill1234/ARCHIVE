...
FileUtils.mkdir("#{ROOT_DIR}/#{OLD_DIR}")
FileUtils.touch("#{ROOT_DIR}/#{OLD_DIR}/#{FILE_NAME}")
...
OLD_SEED_XML = <<-HERE
  <call-function name="vse-crawler-seed-smb">
    ...
    <with name="shares">/testfiles/#{SEED_DIR}/#{OLD_DIR}</with>
  </call-function>
HERE
...
NEW_SEED_XML = <<-HERE
  <call-function name="vse-crawler-seed-smb">
    ...
    <with name="username">gaw</with>
    <with name="password">[[vcrypt]]TMWiymi8UsQ9QvtqWkxuhw==</with>
    <with name="shares">/testfiles/#{SEED_DIR}/#{NEW_DIR}</with>
  </call-function>
HERE
...
FileUtils.mv("#{ROOT_DIR}/#{OLD_DIR}","#{ROOT_DIR}/#{NEW_DIR}")
