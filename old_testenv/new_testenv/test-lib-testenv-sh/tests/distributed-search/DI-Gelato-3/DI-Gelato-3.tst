...
def create_server_file(client_list, lightcrawl, server_file):

   root_dir = os.getenv('TEST_ROOT', '../dstidx-common')

   filename = 'nextclnt'

   cmdstring = "cat " + filename + " | sed -e \'s;__REPLACE_CLIENTS__;"

   for item in client_list:
      if ( isfqdn(item) ):
         fsn = item + ':55555'
      else:
         fsn = item + '.test.vivisimo.com' + ':55555'
      cmdstring = cmdstring + fsn + "\\n"

   cmdstring = cmdstring + ";g\' > " + server_file
...
def create_client_file(my_server, lightcrawl):
   ...
   if ( isfqdn(my_server) ):
      ...
      fsn = my_server + '.test.vivisimo.com' + ':55555'
   ...
   cmdstring = "cat " + filename + " | sed -e \'s;__REPLACE_SERVER__;" + fsn + ";g\' > nextclnt"
...
def set_up_server(server_collection, my_server, client_list, lightcrawl):
...
def set_up_clients(server_collection, client_list):
   ...
   client_collection = server_collection
...
def deathWish(srvr, server_collection, 
   ...
   while ( i < numtocrash ):
      ...
      if ( indexercrash == 2 or indexercrash == 3 ):
         deathangel = doServiceDeath(clnt[i], server_collection,
...
def compare_audit_log_data(srvr, server_collection, clnt):
...
if __name__ == "__main__":
   ...
   #   Define options
   #
   opts, args = getopt.getopt(sys.argv[1:], "IS:C:vlD:d:kE:n:m:Li:R:T:A", ["server=", "client=", "verbose", "last", "data=", "delete=", "keepit", "error=", "number=", "killinterval=", "light", "readonly=", "testname=", "indexatomic", "auditlog"])
   #
   #
   #####################################################################
   #
   ...
   create_client_file(my_server, light_crawl)
   create_server_file(client_list, light_crawl, server_file)

   srvr = set_up_server(server_collection, my_server,
                        client_list, light_crawl)

   srvr.cgi.version_check(8.0)
   srvr.cgi.is_testenv_mounted()

   clnt = set_up_clients(server_collection, client_list)
   ...
   if ( crlro ):
      cnum = 0
      for item in clnt:
         enable_read_only(item, server_collection)
         cnum += 1
