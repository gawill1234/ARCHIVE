...
def Compare_Two(myList, collection, xx, yy, tname, qstring, qfile):
   ...
   #                      query=qstring, filename=qfile, odup="true")
   resp = yy.api_qsearch(source=collection, num=6000,
...
def Like_Collection_Data_Compare(tname="not defined", xx=None,
   ...
   spUrl, spTot, spMis = Compare_Two(spList, collection, xx, yy, tname, qstring, 'qospdump')
   cifsUrl, cifsTot, cifsMis = Compare_Two(cifsList, cifsCollection, xx, yy, tname, qstring, 'qocifsdump')
   if ( xx.TENV.targetos == 'linux' ):
      fsUrl, fsTot, fsMis = Compare_Two(fsList, fsCollection, xx, yy, tname, qstring, 'qofsdump')
