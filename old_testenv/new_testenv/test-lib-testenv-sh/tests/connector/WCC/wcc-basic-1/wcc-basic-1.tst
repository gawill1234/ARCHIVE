...
class doSPUpdate ( Thread ):
   ...
   def __init__(self, site, user, pw, domain, updateList):
      ...
      self.cgi_iface = cgi_interface.CGIINTERFACE()
      self.vapi_iface = vapi_interface.VAPIINTERFACE()
...
#
def Run_The_Test_Case(tname=None, cgi_iface=None, vapi_iface=None,
   ...
   stime = time.time()
   resp = vapi_iface.api_qsearch(source=collection, query="", filename="USout",
...
if __name__ == "__main__":
   ...
   try:
      ...
      if ( nokill ):
         ...
         test_help.Start_The_Crawl_No_Wait()
         i = 0
         while ( i < 5 ):
            time.sleep(45)
            vapi_iface.api_sc_crawler_stop(collection=collection_name,
                                           killit='true',
                                           subc='live')
            time.sleep(10)
            vapi_iface.api_sc_crawler_start(collection=collection_name,
                                            stype='resume')
            i += 1
         cgi_iface.wait_for_idle(collection=collection_name)
   
      using = 0
      current_results = Run_The_Test_Case(tname, cgi_iface, vapi_iface,
   ...
   if (Do_Exit_Routine(tname, total_results, 9) == 0):
      cgi_iface.delete_collection(collection=collection_name, force=1)
      sys.exit(0)
