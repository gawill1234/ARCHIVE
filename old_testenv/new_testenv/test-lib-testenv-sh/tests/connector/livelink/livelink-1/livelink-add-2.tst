...
while getopts "B:U:S:P:C:F:" flag; do
   case "$flag" in
      U) MyUser=$OPTARG;;
      S) MySite=$OPTARG;;
...
if [ -e $cname ]; then
   ...
   java $gt -tname $tname -collection $MyCollection -crawlmode "refresh-inplace" -nosetup -noteardown -query "" $qcnt -query alarm 2 -query clusty 5 -query custom 8 $forwardit
   blah=$?
