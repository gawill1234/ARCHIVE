...
results = TestResults.new('Audit log with index-atomic and offline queue.',
...
def create_docs(count)
  (0...count).map {|i|
    '<index-atomic enqueue-id="%s" ><crawl-url
  ...
  </crawl-data></crawl-url></index-atomic>' %
...
def delete_fields(fields)
  (0...fields.length).map {|i|
    '<index-atomic enqueue-id="%s" ><crawl-delete url="http://adams/doc/%s#%s" /></index-atomic>' %
...
c = Collection.new('audit-log-offline-atomic')
results.associate(c)
c.delete
...
second_fail    = seconds.count {|e| e['status'] != 'successful'}
...
results.add(seconds.length == COUNT,
            "Second field additions: %d audit log entries with %d failures." %
            [seconds.length, second_fail])
