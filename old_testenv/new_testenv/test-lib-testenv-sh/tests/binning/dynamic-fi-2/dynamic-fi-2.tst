...
if __name__ == "__main__":
   ...
   collection_name = "dynamic-fi-2"
   tname = 'dynamic-fi-2'

   urlfile = 'url_to_enqueue'
   ...
   print tname, ":  Content based fast indexing"
   print tname, ":     Multiple uses of the same declaration"
   ...
   xx.run_query(collection=collection_name, query="",
                  ficond='$GOOBER = \"FIDGET\"',
                  defoutput='querywork/fibattleships.wazzat')
   xx.run_query(collection=collection_name, query="",
                  ficond='$GOOBER = \"GermanBattle\"',
                  defoutput='querywork/gbbattleships.wazzat')
   xx.run_query(collection=collection_name, query="",
                  ficond='$GOOBER2 = \"FIDGET\"',
                  defoutput='querywork/gbbattleships2.wazzat')
   ...
   gooval3 = int(xx.count_query_urls(filenm='querywork/gbbattleships.wazzat'))
   gooval4 = int(xx.query_url_count(filenm='querywork/gbbattleships.wazzat'))
   gooval5 = int(xx.count_query_urls(filenm='querywork/gbbattleships2.wazzat'))
   gooval6 = int(xx.query_url_count(filenm='querywork/gbbattleships2.wazzat'))
   ...
   if ( gooval1 != 1 and gooval2 != 1 and
        gooval3 != 1 and gooval4 != 1 and
        gooval5 != 1 and gooval6 != 1):
