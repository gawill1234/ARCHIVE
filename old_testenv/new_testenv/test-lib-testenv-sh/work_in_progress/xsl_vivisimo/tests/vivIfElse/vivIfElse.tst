...
for item in $x; do
   xsltproc $XSLLIB/xslGen.xsl $item > vivIfElse.xsl
   transform -xsl vivIfElse.xsl dummy.xml 2> trashfile >> result.html
   tranres=$?
   if [ $tranres != 0 ]; then
...
echo ""
echo "vivIfElse:  Test Failed, $failtot"
