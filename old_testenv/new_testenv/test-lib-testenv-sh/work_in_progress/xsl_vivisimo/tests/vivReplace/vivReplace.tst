...
for item in $x; do
   xsltproc $XSLLIB/xslGen.xsl $item > vivReplace.xsl
   transform -xsl vivReplace.xsl dummy.xml 2> trashfile >> result.html
   tranres=$?
   if [ $tranres != 0 ]; then
...
echo ""
echo "vivReplace:  Test Failed, $failtot"
