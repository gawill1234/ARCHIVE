...
class Collection
  ...
  def add_converter_in_front(converter_string)
    conf = xml
    ...
    if converters.size == 0
      converters = vse_config.document.create_element('converters')
      converters.add_child(converter_string)
      vse_config.add_child(converters)
    else
      converters.children().first.add_previous_sibling(converter_string)
    end
  ...
  def add_binning_set(binning_set_xml)
    ...
    conf.xpath("/vse-collection/vse-config/vse-index/binning-sets").first.add_child binning_set_xml
    set_xml(conf)
  end
