...
//
//   Ingres compilation:
//      gcc -I $Include_Path table_ins.odbc.c -L $Load_Path -l iiodbc.1
//      where
//         Include_Path = /home/ingres/Ingres/IngresII/ingres/files
//         Load_Path = /home/ingres/Ingres/IngresII/ingres/lib
//      the /home/ingres part is replaceable, depending on where
//      ingres is installed.
//
...
//char *dbname = "WWII_Navy";
char *junk = "WWII_Navy";
//char *junk = "ingres_Ingres";
//char *junk = "SERVER=(local); SERVERTYPE=INGRES; DATABASE=WWII_Navy";
   ...
   if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
         ...
         if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
            retcode = SQLSetConnectAttr(hdbc, SQL_LOGIN_TIMEOUT, &timeouttime, 0);
            if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO) {
               printf("1:  set connection attributes puked\n");
               fflush(stdout);
            }
//            retcode = SQLSetConnectAttr(hdbc, SQL_ATTR_CURRENT_CATALOG, dbname, SQL_NTS);
//            if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO) {
//               printf("2:  set connection attributes puked\n");
//               fflush(stdout);
//            }
            ...
            printf("Failed to Allocate Handle(connector)\n");
            fflush(stdout);
         }
      } else {
         printf("Failed to Set Env Attributes\n");
         fflush(stdout);
      }
   } else {
      printf("Failed to Allocate Handle(environment)\n");
      fflush(stdout);
   }
