...
def CaseEightDataLoad(cnfl_load, file_help):
   ...
   spList = tryit.getAllSpaces()
   print "SPACE LIST"
   print "+++++++++++++++++++++++++++++++++"
   found = False
   for item in spList:
      print item
      if ( item['name'] == 'testenv' ):
         found = True

   if ( found ):
      print "testenv SPACE DELETE FAILED:  testenv space was NOT deleted"
   else:
      print "testenv SPACE DELETE PASSED:  testenv space was deleted as expected"
...
def CaseSixDataLoad(cnfl_load, file_help):
   ...
   tryit = cnfl_load.get_cnfl()
   tryit.commentExistingPage('testenv', 'test_data', 'page has been updated(siz)')
...
def CaseFiveDataLoad(cnfl_load, file_help):
   ...
   tryit = cnfl_load.get_cnfl()
   tryit.commentExistingPage('testenv', 'document', 'page has been updated(five)')
...
def CaseFourDataLoad(cnfl_load, file_help):
   ...
   tryit = cnfl_load.get_cnfl()
   tryit.commentExistingPage('testenv', 'PPT', 'page has been updated(four)')
...
if __name__ == "__main__":
   ...
   if (test_help.Do_Pass_Fail_Routine(total_results, 8, False) == 0):
