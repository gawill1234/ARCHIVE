 ...
 *
 *  TEST TITLE           : Socket system calls regression-UNIXDATAGRAM recvmsg()
 *
 ...
 *
 *			   1) msg_iovlen = 0				positive
 *			   2) bad iov_base address 			negative
 *			   3) MSG_OOB specified				negative
 *                         4) receive operation is interrupted by
 *                            delivery of a signal before any data is
 *                            available for the receive                 negative
 *			   5) nonblocking with no messages available	negative
 *			   6) nonblocking with messages available and
 *			      msg_iovlen = 1				positive
 *			   7) msg_iovlen = MSG_MAXIOVLEN-1		positive
 *			   8) MSG_PEEK flag				positive
 *			   9) MSG_EOR flag				positive
 *			   10)MSG_WAITALL flag				positive
 *			   11)msg_name specified			positive
 *			   12)MSG_TRUNC flag				positive
 ...
 *
 *  INPUT SPECIFICATION  : Usage: unix_recvmsg_d [-d data_file] [-f file] 
 *                         file is the file that the program will bind to.
 *			   The default is "unixd.msg".  data_file is the file
 *			   whose descriptor is passed from one process to
 *			   another process.  The default is "unixd.data".
 *
 ...
 *
 *			   Test case 4) A child process is forked to send a
 *			     signal before any data is received.
 *
 *			   Test case 8) The data should be able to be received
...
char *TCID = "unix_recvmsg_d";	/* Test case identifier */
int TST_TOTAL = 15;		/* Total number of test cases */

/* the default file name for file descriptor which is passed */
static char *data_file="unixd.data";
char *file = "unixd.msg";	/* the default file name for sun_path */
...
main(int argc, char *argv[])
{
  ...
  int c, i;
  int c_sd, s_sd;			/* socket descriptors */
  ...
  void alarm_handler(), check_fd(), check_negative(), check_positive(),
       close_sock(), do_connect(), init_iovec(), logtime(), sigusr1(),
       ul_exit();
  ...
  rmsg.msg_iov = riovec;
  do_connect(&cli, &svr, socklen, &s_sd, &c_sd);
  check_positive(s_sd, c_sd, &rmsg, rflags);
  ...
  rflags = MSG_OOB;
  check_negative(s_sd, &rmsg, rflags, EOPNOTSUPP);
  rflags = 0;
  ...
  default:	/* parent process */
    check_negative(s_sd, &rmsg, rflags, EINTR);
  }
  ...
  if (ioctl(s_sd, FIONBIO, (char *)&on) < 0) {
    ...
    check_negative(s_sd, &rmsg, rflags, EWOULDBLOCK);
  }
  ...
  if (ioctl(s_sd, FIONBIO, (char *)&off) < 0) {
    tst_resm(TBROK, "ioctl(): %s (errno %d)", sys_errlist[errno], errno);
    close_sock(s_sd, c_sd, 0);
    ul_exit();
  }
  ...
  init_iovec(riovec, buf[0], BUFSIZE/2); /* Rcv buffer length<datagram length */
  rflags = MSG_TRUNC;
  check_positive(s_sd, c_sd, &rmsg, rflags);

  /**** Test case 13 ****/

  check_fd(s_sd, c_sd, FALSE);

  /**** Test case 14 ****/

  check_fd(s_sd, c_sd, TRUE);
...
static void
do_connect(cliptr, svrptr, socklen, sd_s, sd_c)
 ...
 *   sd_s, sd_c      	- pointers to socket descriptors returned to caller
 */

{
  ...
  if ((*sd_s = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
  ...
  /*	Do the client connect next	*/

  cliptr->sun_family = AF_UNIX;
  strcpy(cliptr->sun_path, file);

  if ((*sd_c = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
    tst_resm(TBROK, "socket(): %s (errno %d)", sys_errlist[errno], errno);
    if (close(*sd_s) < 0) {
      tst_resm(TWARN, "close(*sd_s): %s (errno %d)", sys_errlist[errno], errno);
    }
    ...
    while (nleft > 0) {
      ...
      if (nread < 0) {
        ...
        if (nread == nleft) {
          ...
          if ((flags == MSG_TRUNC) && (msg->msg_flags != MSG_TRUNC)) {
            tst_resm(TFAIL,"msg_flags = %d, should be set to MSG_TRUNC (%d)",
