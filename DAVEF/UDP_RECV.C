 ...
 *
 *  TEST TITLE           : Socket system calls regression -- UDP recvmsg()
 *
 ...
 *			   9) MSG_WAITALL flag				positive
 *			   10)msg_name specified			positive
 *			   11)MSG_TRUNC flag				positive
 *			   12)blocking with no messages available	positive
 *
 *  CPU TYPES            : CRAY-XMP, CRAY-YMP, CRAY-2, CRAY-C90, CRAY-EL
 *
 *  INPUT SPECIFICATION  : Usage: udp_recvmsg [-P port]
 *                         port is the port number that the program will
 *			   bind to.  The default port number is 41499.
 *
 ...
 *
 *			   Test case 12) Should block until the alarm goes off.
 *
...
char *TCID = "udp_recvmsg";	/* Test case identifier */
int TST_TOTAL = 12;		/* Total number of test cases */
...
main(int argc, char *argv[])
{
  ...
  int port = 41499;			/* the default port number */
  ...
  do_connect(&cli, &svr, socklen, port, &s_sd, &c_sd);
...
static void
do_connect(cliptr, svrptr, socklen, port, sd_s, sd_c)
...

{
  ...
  if ((*sd_s = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
  ...
  if ((*sd_c = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
    tst_resm(TBROK, "socket(): %s (errno %d)", sys_errlist[errno], errno);
    if (close(*sd_s) < 0) {
      tst_resm(TWARN, "close(*sd_s): %s (errno %d)", sys_errlist[errno], errno);
    }
