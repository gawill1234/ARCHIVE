 ...
 *
 *  TEST TITLE           : Socket system calls regression -- TCP bind()
 *
 ...
 *
 *  INPUT SPECIFICATION  : Usage: tcp_bind [-P port]
...
char *TCID= "tcp_bind";		/* Test case identifier */
...
main(int argc, char *argv[])
{
  ...
  sock.sin_port = port;
  check_positive(fd, &sock, sock.sin_port, (short) SOCK_STREAM);
  ...
  if(ioctl(fd2, SIOCGIFCONF, (char *) &ifc) < 0 ) {
...
#ifdef DEBUG
    printf("First ioctl() successful.  Second ioctl will get the interface ");
    printf("address for %s\n", ifc.ifc_req->ifr_name);
#endif DEBUG
    if (ioctl(fd2, SIOCGIFADDR, (char *) ifc.ifc_req) < 0) {
      ...
      check_positive(fd2, &sock, sock.sin_port, (short) SOCK_STREAM);
    }
  }
