...
struct f_list_t *suckit2(workdir, numinode, percent_unlink)
char *workdir;
int numinode, percent_unlink;
{
int error, itercount, main_did_create, fno, do_unlflag;
int maxlink, gotlink;
char *genname();
struct f_list_t *f_list_node, *cr_new_f_list_node(), *put_in_file_list();
struct f_list_t *linktofile;
struct f_list_t *file_head = NULL;
   ...
   while (numinode > ZERO) {
      ...
      if (percent_unlink != ZERO) {
         if (((int)(drand48() * (double)1000)) > 990) {
            if (do_unlflag == ZERO) {
...
int maxlink, gotlink, dir_file_choice, i;
char *genname(), *workdir, **name_list;
struct f_list_t *f_list_node, *cr_new_f_list_node(), *put_in_file_list();
struct f_list_t *linktofile, *tracit;
struct f_list_t *file_head = NULL;

   if ((percent_alt_dirs == 0) && (percent_dirs == 0)) {
      file_head = suckit2(homedir, numinode, percent_unlink);
      return(file_head);
   }
   ...
   while (numinode > ZERO) {
       ...
       */
      if (percent_unlink != ZERO) {
         if (((int)(drand48() * (double)1000)) > 970) {
            if (do_unlflag == ZERO) {
               do_unlflag = do_unlinkit(file_head, percent_unlink);
            }
         }
