   ...
   for (i = 0; i < fs_node->blk_counter; i++) {
      if (fs_node->brk_blk[i].dstry != NO_DESTROY) {
         ...
         switch (fs_node->brk_blk[i].dstry_blk) {
            case ON_DISK_SUPER:
                 set_dsr_dstry(fs_node, i);
                 break;

            case IN_CORE_SUPER:
                 set_msr_dstry(fs_node, i);
                 break;

            case IN_CORE_INODE:
                 set_ino_dstry(fs_node, i);
                 break;

            case IN_CORE_MOUNT:
                 set_mnt_dstry(fs_node, i);
                 break;

            case IN_CORE_DYNAMIC:
                 set_dyn_dstry(fs_node, i);
                 break;
...
long *get_good_data();
   ...
   switch (fs_node->brk_blk[bn].dstry) {
      case MAGIC_NUMBER:
         ...
         fs_node->brk_blk[bn].good_data = &nc1dbmagic;
         fs_node->brk_blk[bn].offset = fs_node->dyn_offset;
         break;
   
      case RANDOM_WORD:
         fs_node->brk_blk[bn].offset =
                 random_offset(fs_node->dyn_offset, sizeof(struct nc1dblock));
         fs_node->brk_blk[bn].good_data =
                 get_good_data(fs_node->brk_blk[bn].offset, KMEM,
                               fs_node->brk_blk[bn].bd_count);
         break;
   }
}

int set_ino_dstry(fs_node, bn)
struct sprblk *fs_node;
int bn;
{
long *get_good_data();
   ...
   switch (fs_node->brk_blk[bn].dstry) {
      case MAGIC_NUMBER:
         fs_node->brk_blk[bn].good_data = (long *)malloc(sizeof(long));
         fs_node->brk_blk[bn].good_data = &nc1inomagic;
         fs_node->brk_blk[bn].offset = fs_node->ino_offset + (4 * 8);
         break;
...
long *get_good_data();
   ...
   switch (fs_node->brk_blk[bn].dstry) {
      case MAGIC_NUMBER:
         fs_node->brk_blk[bn].good_data = (long *)malloc(sizeof(long));
         fs_node->brk_blk[bn].good_data = &nc1magic;
         fs_node->brk_blk[bn].offset = fs_node->mem_offset;
         break;
...
long *get_good_data();
   ...
   switch (fs_node->brk_blk[bn].dstry) {
      case MAGIC_NUMBER:
         fs_node->brk_blk[bn].good_data = (long *)malloc(sizeof(long));
         fs_node->brk_blk[bn].good_data = &nc1magic;
         fs_node->brk_blk[bn].offset = fs_node->dsk_offset;
         break;
      ...
      case RANDOM_WORD:
         ...
         fs_node->brk_blk[bn].good_data = 
                 get_good_data(fs_node->brk_blk[bn].offset, fs_node->fs_device,
                               fs_node->brk_blk[bn].bd_count);
         break;
   }
}

int set_mnt_dstry(fs_node, bn)
struct sprblk *fs_node;
int bn;
{
long *get_good_data();
   ...
   switch (trans_for) {
      ...
      case IN_CORE_INODE:
               if (strcmp(opt_name, "magic") == 0) {
                  new_fs_node->brk_blk[new_fs_node->blk_counter].dstry = 1;
                  new_fs_node->brk_blk[new_fs_node->blk_counter].bd_count = 1;
                  break;
               }
      case IN_CORE_MOUNT:
               if (strncmp(opt_name, "random", 6) == 0) {
                  new_fs_node->brk_blk[new_fs_node->blk_counter].dstry = 2;
               ...
               if (strncmp(opt_name, "offset", 6) == 0) {
                  new_fs_node->brk_blk[new_fs_node->blk_counter].dstry = 3;
                  if (strlen(opt_name) <= 7) {
                    new_fs_node->brk_blk[new_fs_node->blk_counter].bd_count = 1;
                  }
                  else {
